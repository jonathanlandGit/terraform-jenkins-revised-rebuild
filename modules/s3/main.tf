data "aws_s3_bucket" "jenkins_filez" {
  bucket = "jenkins-prod-cg-copy"
}

resource "aws_s3_bucket_object" "env_file" {
  bucket = data.aws_s3_bucket.jenkins_filez.id
  key    = "jenkins.env"
...
data "aws_s3_bucket_object" "init_file" {
  bucket = data.aws_s3_bucket.jenkins_filez.id
  key    = "init.groovy"
...
data "aws_s3_bucket_object" "plugin_file" {
  bucket = data.aws_s3_bucket.jenkins_filez.id
  key    = "plugins.txt"
...
data "aws_s3_bucket_object" "jobs_file" {
  bucket = data.aws_s3_bucket.jenkins_filez.id
  key    = "jobs.tgz"
